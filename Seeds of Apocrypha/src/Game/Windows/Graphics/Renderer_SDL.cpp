...
void Renderer_SDL::DrawTxt(Text& txt) {
	//Cast to SDL
	Text::Info* ti = &txt.info;
	Font_SDL* fntsdl = static_cast<Font_SDL*>(txt.font);
	...
	surface = TTF_RenderText_Shaded_Wrapped(fntsdl->GetFont(), ti->str.c_str(), ti->str.size(), c, SDL_Color{ 0,0,0,0 }, ti->max_width);
	if (!surface) {

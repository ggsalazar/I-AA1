...
void Renderer_D2D::DrawSheet(const Sprite& sheet, const Vector2i& pos) {
	...
	render_target->DrawBitmap(sheetd2d->GetBitmap(), D2D1::RectF(pos.x, pos.y, pos.x + sheet.GetSheetSize().x, pos.y + sheet.GetSheetSize().y));
}
...
void Renderer_D2D::DrawSprite(const Sprite& spr) {
	...
	//Set draw location
	Vector2f pos = { round((si->pos.x - (si->spr_size.x * si->origin.x))),
...
void Renderer_D2D::DrawTxt(Text& txt) {
	...
	D2D1_POINT_2F posd2d = D2D1::Point2F(round((ti->pos.x - (ti->str_size.x * ti->origin.x))),
...
void Renderer_D2D::DrawLine(const Line& line, const Color& color, const uint edge_w) {
	...
	render_target->DrawLine(D2D1::Point2F(line.pos1.x,
		line.pos1.y),
